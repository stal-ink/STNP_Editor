...
STNP_Result STNP_Notify_Dispatch(STNP_U8 source, STNP_U8 notify_code, STNP_U16 result, const STNP_U8 *payload, STNP_U8 length)
{
    ...
    if (STNP_NotifyDispatchReceive_IsEnabled() != 0U)
    {
        ...
        {
            return Chassis_NotifyDispatch(&g_chassismain, notify_code, result, payload, length);
        }
        if (source == STNP_INSTANCE_MOTORMAIN_ID)
        {
            return Motor_NotifyDispatch(&g_motormain, notify_code, result, payload, length);
        }
        if (source == STNP_INSTANCE_SENSORFRONT_ID)
        {
            return Sensor_NotifyDispatch(&g_sensorfront, notify_code, result, payload, length);
        }
        if (source == STNP_INSTANCE_SENSORREAR_ID)
        {
            return Sensor_NotifyDispatch(&g_sensorrear, notify_code, result, payload, length);
        }
    }
    return STNP_OK;
}
