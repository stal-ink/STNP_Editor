...
def make_env(template_root: Path | None = None) -> Environment:
    """构造 C 模板环境；模板目录不可用时映射为 ``E4009``。"""
    try:
        root = template_root or (resources_root() / settings.template_dir("c"))
        env = Environment(
        ...
        env.filters["hex2"] = lambda v: f"0x{int(v):02X}U"
        env.filters["hex4"] = lambda v: f"0x{int(v):04X}U"
    except (TemplateError, OSError) as exc:
        raise GenerationError("E4009", message=f"C 模板环境初始化失败: {exc}") from exc
    return env
